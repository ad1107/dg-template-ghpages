...
// Helper function to get path prefix
function getPathPrefix() {
  return process.env.ELEVENTY_ENV === "prod" ? (process.env.PATH_PREFIX || "") : "";
}
...
function transformImage(src, cls, alt, sizes, widths = ["500", "700", "auto"]) {
  let options = {
    ...
    urlPath: getPathPrefix() + "/img/optimized",
  };
...
function getAnchorAttributes(filePath, linkTitle) {
  ...
  // Add path prefix to permalink for production
  const finalPermalink = getPathPrefix() + permalink;
  ...
  return {
    attributes: {
      ...
      "href": `${finalPermalink}${headerLinkPath}`,
    },
    innerHTML: title,
  }
}
...
module.exports = function (eleventyConfig) {
  ...
  return {
    pathPrefix: process.env.ELEVENTY_ENV === "prod" ? (process.env.PATH_PREFIX || "") : "",
    dir: {
      input: "src/site",
      output: "dist",
      data: `_data`,
